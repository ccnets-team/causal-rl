...
class EnvObservation:
    def __init__(self, obs_shapes, obs_types, num_agents, max_seq_len):
        ...
        self.mask = np.zeros((self.num_agents, self.max_seq_len), dtype=np.float32)

    def _create_empty_data(self):
        observations = {}
        for obs_type, shape in zip(self.obs_types, self.obs_shapes):
            observations[obs_type] = np.zeros((self.num_agents, self.max_seq_len, *shape), dtype=np.float32)
        return observations
    ...
    def __getitem__(self, key):
        ...
        new_observation = EnvObservation(self.obs_shapes, self.obs_types, num_agents=new_num_agents, max_seq_len=new_model_seq_length)
